...
- RxSwift와 MVVM을 기반으로, Input/Output 패턴을 적용한다.

- 비즈니스 로직에 필요한 이벤트와 데이터를 Input으로 전달하고,
    
    transform 메서드를 통해 View에 반영해야 하는 이벤트와 데이터를 Output으로 반환한다
...
- 이벤트에 대한 비동기 처리를 적절히 하기 위해 RxSwift 를 활용해 필요한 기능을 구현하였다.

1. SearchBar
    1. 검색 화면
        - 검색 버튼을 누르면 네트워크 통신이 이루어지는 구조
        - `throttle` 키워드를 이용해 중복 클릭을 방지하여 네트워크 과부하가 일어나지 않도록 한다.
...
1. refreshControl
- 이번 프로젝트에서는 다양한 방식으로 화면들의 동기화 과정을 구상하였다.
- 보관함 화면에서는 앱 받기/삭제를 했을 때 즉각적으로 화면에 반영되지 않고,
    
    **refreshControl**을 통해 최신 데이터를 불러올 수 있도록 했다.
...
- 섹션이 하나인 경우에는 RxSwift의 dataSource로 구현이 가능하지만,
    
    여러 섹션을 다루기 위해 **RxDataSource** 를 활용하였다
...
- tableView의 items를 그려주는 부분이 RxSwift로 구현하였더니, items에 변화가 없으면 다시 그리질 않는다.
...
- 셀 위의 버튼을 Input으로 주기 위해서는 해당 셀에 대한 데이터도 같이 전달해야 하는데 그 과정에서 실패..
...
1. 모든 ViewController의 `viewWillAppear` 에서 Realm 데이터를 확인하고, 이를 반영한다.
    - 굳이 확인하지 않아도 되는 경우에도 데이터를 다시 불러오기 때문에 리소스가 낭비된다고 생각했다.
2. 싱글톤 패턴으로 모든 클래스에서 접근 가능한 인스턴스 변수를 하나 생성해서, 데이터의 변화를 감지한다.
    - 다른 역할 없이 특정 데이터를 감지하는 역할로서 싱글톤 인스턴스를 메모리에 유지시키는 게 낭비라고 생각했다.

- 내가 확인해야 하는 것은 **Realm의 데이터가 업데이트 되었는가** 이다.
- **Realm Notification** 기능을 활용해서 해결하였다.
...
- Input / Output 패턴을 사용하면 특정 이벤트가 발생함을 `ControlEvent<>` 로 전달해준다.
- RxCocoa에서 제공하는 `tableView.rx.itemSelected`, `refreshControl.rx.controlEvent(.valueChanged)`는 그대로 전달해주면 되지만, 이러한 메서드가 없을 때는 시점을 어떻게 전달해줘야 하는지 고민했다.

- Reactive 클래스를 확장해서 새로운 메서드를 선언하는 방법을 알게 되었다.
